...
use neo_contract::{builders::ContractCall, BaseContract, ContractError};
...
///
/// ```no_run
/// use neo_middleware::{SignerMiddleware, transformer::DsProxy};
/// use neo_signers::LocalWallet;
/// use neo_providers::{Provider, Http};
/// use neo_types::{Address, Bytes};
...
impl DsProxy {
	...
	/// already been deployed to the neo network.
...
impl DsProxy {
	/// Deploys a new DsProxy contract to the neo network.
	pub async fn build<M: Middleware, C: Into<Arc<M>>>(
		...
		// Fetch network magic and the corresponding address of DsProxyFactory contract
		...
		let factory: Address = match factory {
			Some(addr) => addr,
			None => {
				let network_magic = client
					.get_network_magic()
					.await
					.map_err(ContractError::from_middleware_error)?;
				match ADDRESS_BOOK.get(&network_magic) {
                    Some(addr) => *addr,
                    None => panic!(
                        "Must either be a supported network magic or provide DsProxyFactory contract address"
                    ),
                }
			},
		};
