...
use neo_providers::{Http, Provider, Ws};
...
pub fn get_wallet(anvil: &AnvilInstance, idx: usize) -> LocalWallet {
	LocalWallet::from(anvil.keys()[idx].clone()).with_network_magic(anvil.network_magic())
}

// TODO: Move this to `neo-tests`
#[tokio::test]
async fn test_derive_eip712() {
	...
	mod contract {
		neo_contract::abigen!(
			DeriveEip712Test,
			"./neo-contract/tests/solidity-contracts/DeriveEip712Test.json",
	...
	let contract: contract::DeriveEip712Test<_> =
		contract::DeriveEip712Test::deploy(client.clone(), ())
			.unwrap()
			.send()
			.await
			.unwrap();
