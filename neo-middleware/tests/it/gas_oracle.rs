...
use neo_middleware::gas_oracle::{
	BlockNative, EndPoint, Etherchain, GasCategory, GasNow, GasOracle, GasOracleError,
...
#[allow(deprecated)]
async fn neo_gas_station() {
	let neo_gas_station_oracle = neo_middleware::gas_oracle::EthGasStation::default();
	let gas_price = neo_gas_station_oracle.fetch().await.unwrap();
	assert!(gas_price > U256::zero());
}
...
async fn neocan() {
	...
	let neocan_client = Client::new_from_opt_env(chain).unwrap();

	// initialize and fetch gas estimates from EndPoint
	// since neocan does not support `fastest` category, we expect an error
	let neocan_oracle = EndPoint::new(neocan_client.clone()).category(GasCategory::Fastest);
	let error = neocan_oracle.fetch().await.unwrap_err();
	assert!(matches!(error, GasOracleError::GasCategoryNotSupported));

	// but fetching the `standard` gas price should work fine
	let neocan_oracle = EndPoint::new(neocan_client).category(GasCategory::SafeLow);

	let gas_price = neocan_oracle.fetch().await.unwrap();
	assert!(gas_price > U256::zero());
	let ten_neo = parse_ether(10).unwrap();
	assert!(gas_price < ten_neo, "gas calculation is wrong (too high)");
}
