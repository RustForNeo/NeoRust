...
impl NeoToken{
    ...
    async fn unclaimed_gas_contract(&self, script_hash: &H160, block_height: i32) -> Result<i64, ContractError> {
        self.call_function_returning_int("unclaimedGas", vec![script_hash.into(), block_height.into()])
            .await
    }

    // Candidate Registration

    fn register_candidate(&self, candidate_key: &ECPublicKey) -> Result<TransactionBuilder, ContractError> {
        self.invoke_function("registerCandidate", vec![candidate_key.to_stack_item()])
    }

    fn unregister_candidate(&self, candidate_key: &ECPublicKey) -> Result<TransactionBuilder, ContractError> {
...
// Voting
    ...
    async fn vote(&self, voter: &H160, candidate: Option<&ECPublicKey>) -> Result<TransactionBuilder, ContractError> {
    ...
    fn build_vote_script(&self, voter: &H160, candidate: Option<&ECPublicKey>) -> Result<Vec<u8>, ContractError> {
        let params = match candidate {
            Some(key) => vec![voter.into(), key.to_stack_item()],
            None => vec![voter.into(), StackItem::null()],
        };
