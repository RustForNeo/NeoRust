...
//
// 		stream::unfold(initial_index, move |last_index| {
...
// 		polling_interval: i32,
// 	) {
// 		let index = self.block_index_publisher(polling_interval)
// 			.await;
// 		index.for_each(|index| async move {
// 			let binding = NEO_INSTANCE.read().unwrap();
// 			let req = binding.get_block_by_index(index as u32, full_transaction_objects);
// 			let block = req.request().await.unwrap();
// 		}).await;
//
//
// 		// let req = binding.get_block_by_index(index as u32, full_transaction_objects);
...
//
// 		let binding = NEO_INSTANCE.read().unwrap();
// 		let stream = iter(blocks.into_iter().map(async move |block| {
// 			// Assuming NEO_INSTANCE is globally accessible and has type RwLock<...>
// 			let req = binding
...
// 		}));
//
// 		stream.buffer_unordered(10).boxed()
// 	}
//
