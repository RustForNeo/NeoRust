...
impl SignatureData {
    ...
    pub fn sign_message(message: &Bytes, key_pair: &mut KeyPair) -> Result<SignatureData, NeoError> {
        let signature = key_pair.private_key.sign(&Sha256::digest(message));

        let mut rec_id = None;
        for i in 0..4 {
            if let Some(key) = key_pair.public_key.recover(&i, &signature, &Sha256::digest(message))? {
        ...
        let rec_id = rec_id.ok_or(NeoError::Runtime("Could not construct recoverable key".to_string()))?;
