...
impl Account {
    ...
    pub fn new(address: Address, label: Option<String>, verification_script: Option<VerificationScript>, signing_threshold: Option<i32>, nr_of_participants: Option<i32>) -> Self {
        Self {
            key_pair: None,
            address,
            label,
    ...
    pub fn from_key_pair(key_pair: KeyPair, signing_threshold: Option<i32>, nr_of_participants: Option<i32>) -> Result<Self, WalletError> {
    ...
    pub fn from_key_pair_opt(key_pair: Option<KeyPair>, address: Address, label: Option<String>, verification_script: Option<VerificationScript>, is_locked: bool, encrypted_private_key: Option<String>, wallet: Option<Wallet>, signing_threshold: Option<i32>, nr_of_participants: Option<i32>) -> Self {
        Self {
            key_pair,
            address,
            label,
    ...
    pub fn from_nep6_account(nep6_account: &NEP6Account) -> Result<Self, WalletError> {
        let (verification_script, signing_threshold, nr_of_participants) = match nep6_account.contract {
    ...
    pub fn decrypt_private_key(&mut self, password: &str) -> Result<(), WalletError> {
        ...
        let encrypted_private_key = self.encrypted_private_key.as_ref().ok_or(WalletError::AccountState("No encrypted private key present".to_string()))?;
    ...
    pub fn encrypt_private_key(&mut self, password: &str) -> Result<(), WalletError> {
        let key_pair = self.key_pair.as_ref().ok_or(WalletError::AccountState("No decrypted key pair present".to_string()))?;
    ...
    pub fn get_signing_threshold(&self) -> Result<i32, WalletError> {
        self.signing_threshold.ok_or_else(|| WalletError::AccountState("Account is not multisig".to_string()))
    }

    pub fn get_nr_of_participants(&self) -> Result<i32, WalletError> {
        self.nr_of_participants.ok_or_else(|| WalletError::AccountState("Account is not multisig".to_string()))
    }
    ...
    pub fn to_nep6_account(&self) -> Result<NEP6Account, WalletError> {
        if self.key_pair.is_some() && self.encrypted_private_key.is_none() {
            return Err(WalletError::AccountState("Account private key is decrypted but not encrypted".to_string()));
        }
    ...
    pub fn from_verification_script(script: &VerificationScript) -> Result<Self, WalletError> {
        let address = H160::from_script(&script.to_bytes()?)
            .to_address();
    ...
    pub fn create_multisig(public_keys: &[ECPublicKey], signing_threshold: i32) -> Result<Self, WalletError> {
