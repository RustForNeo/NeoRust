...
impl ScriptBuilder {
    ...
    pub fn push_param(&mut self, param: &Option<ContractParameter>) -> Result<&mut Self, dyn Error> {
        match param {
    ...
    fn pad_number(n: i128, size: usize) -> Bytes {
        ...
        if n.is_negative() {
            let mut padding = Bytes::from_iter(std::iter::repeat(pad_byte).take(size - bytes.len()));
    ...
    pub fn push_map(&mut self, map: &HashMap<ContractParameter, ContractParameter>) -> Result<&mut Self, dyn Error> {
        ...
        Ok(self.push_integer(map.len() as i64)?
