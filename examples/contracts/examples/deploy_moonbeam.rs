...
async fn main() -> eyre::Result<()> {
	...
	// manifest of `neo`. which will be `../` relative to this file
	...
	let key = neo::core::utils::moonbeam::dev_keys()[0].clone();

	// 2. instantiate our wallet with network magic
	let wallet: LocalWallet = LocalWallet::from(key).with_network_magic(Chain::MoonbeamDev);
