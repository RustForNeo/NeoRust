...
async fn mocked_block_number() -> eyre::Result<()> {
	...
	// Act
	let ret_block_3: U64 = JsonRpcClient::request(&mock, "neo_blockNumber", ()).await?;
	let ret_block_2: U64 = JsonRpcClient::request(&mock, "neo_blockNumber", ()).await?;
	let ret_block_1: U64 = JsonRpcClient::request(&mock, "neo_blockNumber", ()).await?;
