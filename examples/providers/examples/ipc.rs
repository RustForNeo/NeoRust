...
async fn main() -> eyre::Result<()> {
	...
	let wneo_usdc = IUniswapV2Pair::new(pair_address, provider.clone());

	let block = provider.get_block_number().await?;
	println!("Current block: {block}");

	let mut initial_reserves = wneo_usdc.get_reserves().call().await?;
	...
	while let Some(block) = stream.next().await {
		...
		let reserves = wneo_usdc.get_reserves().call().await?;
